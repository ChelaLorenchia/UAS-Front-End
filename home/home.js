...
app.controller('SearchController', ['$scope', '$window', function ($scope, $window) {
    ...
    $scope.searchAndRedirect = function () {
        const result = $scope.files.find(file =>
            file.name.toLowerCase().includes($scope.query.toLowerCase())
        );
    ...
    $scope.updateRecommendations = function () {
        if ($scope.query.trim()) {
            $scope.recommendations = $scope.files.filter(file =>
                file.name.toLowerCase().includes($scope.query.toLowerCase())
            );
        } else {
            $scope.recommendations = [];
    ...
    $scope.selectRecommendation = function (url) {
        $window.location.href = url;
    };

    $scope.clearRecommendations = function (event) {
    ...
    document.addEventListener('click', $scope.clearRecommendations);
}]);